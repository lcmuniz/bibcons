...
public interface EdicaoRepository extends JpaRepository<Edicao, Integer> {
    ...
    where (upper(ae.titulo) like :filtro or upper(ae.subtitulo) like :filtro or upper(ae.autores) like :filtro or upper(ae.assuntos) like :filtro or upper(ae.resumo) like :filtro) and (e.ativo = 'S') order by ae.titulo""", nativeQuery = true)
    List<ArtigoEdicao> filtrar(String filtro);
